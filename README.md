<h1 align="center" style="border-bottom: none;">Empty App</h1>

<h3 align="center">The simple HTML sandbox in your browser developer tools</h3>
...
<h2 align="center">:thinking: The reason?</h2>

It is a simple `<body />` tag in the browser. I created the empty "website" because I needed the simplest HTML sandbox in a real browser without any `<iframe />` which are usually used by CodePen or other sandbox apps. It helps me to fastly develop or test the simplest HTML and CSS structures directly in the browser using the default Developer Tools.
<br />
And yeah, I called the empty `<body />` tag - App. It is not a full-featured app like CodePen, but it has a very simple and very useful feature: DOM state save.
When you change the DOM, everything in the `<body />` tag will be kept on page refresh. So you can close the browser and continue your work later :cherries:

<br />

> Go to Empty App: [emptyapp.now.sh](https://emptyapp.now.sh/)
...
To clear the page. In the console use: `app.clear()`

Any changes in the `<body />` will be kept on page refresh

The `<script />` tags are supported, but before using it, the recommendation is to disable autosave (which is enabled by default), because of possible DOM changes by JavaScript. Disable autosave with `app.autosave(false)`

To save changes manually: `app.save()`

If you want to reset page (show default message), use command: `app.reset()`
